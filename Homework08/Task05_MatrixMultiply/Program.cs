...
int[,] Fill2Array(int[,] array) // заполнение матрицы случайными числами
{
...
void Print2ArrayConsole(int[,] array) // вывод матрицы на печать
{
...
bool IsMatrixCompatible(int[,]A, int[,]B) // проверка матриц на совместимость к перемножению
{
    if (A.GetLength(1) != B.GetLength(0)) return false;
    else return true;
}

int[,] MultiMatrix(int[,] A, int[,] B) // метод перемножения матриц
{
...
int n2 = 3; // количество столбцов матрицы В
int[,] A = new int[m1, n1];
int[,] B = new int[m2, n2];
